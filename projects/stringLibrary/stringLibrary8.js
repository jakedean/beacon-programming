class StringLibrary {
  ...
  /**
   * Take a string in and return the reversed version of that string:
   * ('jake') -> 'ekaj'
   *
   * @param {String} str The string we are trying to reverse.
   */
  reverse(str) {
    var reversedString = '';
    // Loop through from the right to the left of the stirng and create our new
    // string one character at a time.
    for (var strIndex = str.length - 1; strIndex >= 0; strIndex--) {
      reversedString += str[strIndex];
    }
    return reversedString;
  }

  /**
   * Truncate or "cut off" the string at a given length.
   * ('jake', 2) -> 'ja'
   * ('jake', 100) -> 'jake'
   *
   * @param {String} str              The string we are looking to truncate at a given length.
   * @param {Number} truncationLength The max length of the string we are looking to create.
   */
  truncate(str, truncationLength) {
    var truncatedString = '';
    for (var strIndex = 0; strIndex < str.length; strIndex++) {
      var lengthToThisIndex = strIndex + 1;
      // ONLY add this character to our truncatedString IF we are still under our truncation length.
      if (lengthToThisIndex <= truncationLength) {
        truncatedString += str[strIndex];
      }
    }
